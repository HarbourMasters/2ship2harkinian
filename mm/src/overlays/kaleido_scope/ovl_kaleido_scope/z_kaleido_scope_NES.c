...
void KaleidoScope_DrawPages(PlayState* play, GraphicsContext* gfxCtx) {
    ...
    if (!IS_PAUSE_STATE_GAMEOVER) {
        ...
        if ((pauseCtx->pageIndex != PAUSE_ITEM) && (pauseCtx->pageIndex != PAUSE_QUEST)) {
            ...
            POLY_OPA_DISP = KaleidoScope_DrawPageSections(POLY_OPA_DISP, pauseCtx->itemPageVtx, GET_REGION_TEX(sItemPageBgTextures));
        ...
        if ((pauseCtx->pageIndex != PAUSE_QUEST) && (pauseCtx->pageIndex != PAUSE_ITEM)) {
            ...
            POLY_OPA_DISP = KaleidoScope_DrawPageSections(POLY_OPA_DISP, pauseCtx->questPageVtx, GET_REGION_TEX(sQuestPageBgTextures));
        ...
        if ((pauseCtx->pageIndex != PAUSE_MASK) && (pauseCtx->pageIndex != PAUSE_MAP)) {
            ...
            POLY_OPA_DISP = KaleidoScope_DrawPageSections(POLY_OPA_DISP, pauseCtx->maskPageVtx, GET_REGION_TEX(sMaskPageBgTextures));
        ...
        switch (pauseCtx->pageIndex) {
            case PAUSE_ITEM:
                if (pauseCtx->mainState <= PAUSE_MAIN_STATE_EQUIP_MASK) {
                    ...
                    POLY_OPA_DISP =
                        KaleidoScope_DrawPageSections(POLY_OPA_DISP, pauseCtx->itemPageVtx, GET_REGION_TEX(sItemPageBgTextures));
            ...
            case PAUSE_QUEST:
                ...
                POLY_OPA_DISP =
                    KaleidoScope_DrawPageSections(POLY_OPA_DISP, pauseCtx->questPageVtx, GET_REGION_TEX(sQuestPageBgTextures));
            ...
            case PAUSE_MASK:
                ...
                POLY_OPA_DISP =
                    KaleidoScope_DrawPageSections(POLY_OPA_DISP, pauseCtx->maskPageVtx, GET_REGION_TEX(sMaskPageBgTextures));
    ...
    if (CVarGetInteger("gEnhancements.Kaleido.PauseSave", 0) || CVarGetInteger("gEnhancements.Kaleido.GameOver", 0)) {
        ...
        if ((pauseCtx->state == PAUSE_STATE_SAVEPROMPT) || IS_PAUSE_STATE_GAMEOVER) {
            ...
            } else if (((pauseCtx->state == PAUSE_STATE_SAVEPROMPT) &&
            ...
            } else if ((pauseCtx->state == PAUSE_STATE_GAMEOVER_CONTINUE_PROMPT) ||
            ...
            if ((pauseCtx->state != PAUSE_STATE_GAMEOVER_CONTINUE_PROMPT) &&
                (pauseCtx->state != PAUSE_STATE_GAMEOVER_10)) {
...
void KaleidoScope_UpdateOpening(PlayState* play) {
    ...
    if (pauseCtx->switchPageTimer == 64) {
        ...
        if (pauseCtx->cursorSpecialPos == 0) {
            ...
            // #region 2S2H [Dpad]
            gSaveContext.shipSaveContext.dpad.status[EQUIP_SLOT_D_RIGHT] =
                gPageSwitchNextButtonStatus[pauseCtx->pageIndex][1];
            gSaveContext.shipSaveContext.dpad.status[EQUIP_SLOT_D_LEFT] =
                gPageSwitchNextButtonStatus[pauseCtx->pageIndex][1];
            gSaveContext.shipSaveContext.dpad.status[EQUIP_SLOT_D_DOWN] =
                gPageSwitchNextButtonStatus[pauseCtx->pageIndex][1];
            gSaveContext.shipSaveContext.dpad.status[EQUIP_SLOT_D_UP] =
                gPageSwitchNextButtonStatus[pauseCtx->pageIndex][1];
            // #endregion
            gSaveContext.buttonStatus[EQUIP_SLOT_A] = BTN_ENABLED;
        } else {
