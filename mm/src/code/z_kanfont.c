...
void Font_LoadChar(PlayState* play, u16 codePointIndex, s32 offset) {
    ...
    // DmaMgr_SendRequest0(&font->charBuf[font->unk_11D88][offset], SEGMENT_ROM_START(kanji) +
    // func_80184AA0_cj0(codePointIndex), 0x80);
}
...
void Font_LoadOrderedFont_JP(Font* font) {
    ...
    while ((font->msgBuf.wchar[var_s1] != 0x500)) {
        ...
        if ((font->msgBuf.wchar[var_s1] != 0xA) && (font->msgBuf.wchar[var_s1] != 9) &&
            (font->msgBuf.wchar[var_s1] != 0x2000)) {
