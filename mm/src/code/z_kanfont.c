...
void Font_LoadCharNES(PlayState* play, u8 codePointIndex, s32 offset) {
    ...
    if (fontIdx >= 0 && fontIdx < ARRAY_COUNT(gFontTbl)) {
        memcpy(&font->charBuf[font->unk_11D88][offset], gFontTbl[fontIdx], strlen(gFontTbl[fontIdx]) + 1);
    }
...
static u8 sFontOrderingPAL[] = {
    0x10, 0x11, 0x12, 0x13, 0x14, 0x15, 0x16, 0x17, 0x18, 0x19, 0x21, 0x22, 0x23, 0x24, 0x25, 0x26, 0x27, 0x28, 0x29,
    0x2A, 0x2B, 0x2C, 0x2D, 0x2E, 0x2F, 0x30, 0x31, 0x32, 0x33, 0x34, 0x35, 0x36, 0x37, 0x38, 0x39, 0x3A, 0x41, 0x42,
    0x43, 0x44, 0x45, 0x46, 0x47, 0x48, 0x49, 0x4A, 0x4B, 0x4C, 0x4D, 0x4E, 0x4F, 0x50, 0x51, 0x52, 0x53, 0x54, 0x55,
    0x56, 0x57, 0x58, 0x59, 0x5A, 0x00, 0x0D, 0x0E, 0x1A, 0x60, 0x61, 0x62, 0x63, 0x64, 0x65, 0x66, 0x67, 0x68, 0x69,
...
void Font_LoadOrderedFont(Font* font) {
    ...
    u8* fontOrdering = (gameRegion == GAME_REGION_PAL) ? sFontOrderingPAL : sFontOrdering;
    u8 endFont = (gameRegion == GAME_REGION_PAL) ? 0x8F : 0x8C;
    // #endregion

    while (1) {
        writeLocation = &font->fontBuf[codePointIndex * FONT_CHAR_TEX_SIZE];
        // #region 2S2H [Port]
        loadOffset = fontOrdering[codePointIndex]; // * FONT_CHAR_TEX_SIZE;
        if (fontOrdering[codePointIndex] == 0) {
            loadOffset = 0;
        }

        // DmaMgr_SendRequest0(writeLocation, (uintptr_t)SEGMENT_ROM_START(nes_font_static) + loadOffset,
        // FONT_CHAR_TEX_SIZE);
        memcpy(writeLocation, gFontTbl[loadOffset], strlen(gFontTbl[loadOffset]) + 1);
        // #endregion

        if (fontOrdering[codePointIndex] == endFont) {
            break;
        }
        codePointIndex++;
    }
