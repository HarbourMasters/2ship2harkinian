...
void AnimationContext_SetLoadFrame(PlayState* play, PlayerAnimationHeader* animation, s32 frame, s32 limbCount,
    ...
    if (entry != NULL) {
        ...
        // 2S2H [Port] sometimes a HESS can set a negative frame value from a negative playback speed. When converted to a signed value this will cause a crash due to copying way much data.
        if (frame < 0) {
            frame = 0;
        }
