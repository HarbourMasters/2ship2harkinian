	...
	Regions[RR_MISCELLANEOUS] = RandoRegion{ .name = "Various Regions", .sceneId = SCENE_SPOT00,
		.checks = {
			...
			CHECK(RC_ENEMY_DROP_BLUE_BUBBLE,	CanKillEnemy(ACTOR_EN_BB)			&& (CanReachRegions({ RR_TERMINA_FIELD,
																										 ...
																										 RR_ROAD_TO_IKANA_BELOW_LEDGE }) && IS_NIGHT() ||
																										 CanReachRegions({ RR_IKANA_CASTLE_BUBBLE_ROOM,
			...
			CHECK(RC_ENEMY_DROP_BOMBCHU,		CanKillEnemy(ACTOR_EN_RAT)			&& CanReachRegions({ RR_TERMINA_FIELD,
																										 RR_ROAD_TO_IKANA_FIELD_SIDE,
																										 RR_ROAD_TO_IKANA_BELOW_LEDGE }) && IS_DAY()),
			// TODO: Some spawns in Termina Field are day only, but that won't matter unless and until those regions are split up.
			CHECK(RC_ENEMY_DROP_CHU,			CanKillEnemy(ACTOR_EN_SLIME)		&& (CanReachRegions({ RR_ROAD_TO_SOUTHERN_SWAMP }) && IS_DAY() ||
																										 CanReachRegions({ RR_TERMINA_FIELD,
			...
			CHECK(RC_ENEMY_DROP_DEKU_BABA,		CanKillEnemy(ACTOR_EN_DEKUBABA)		&& CanReachRegions({ RR_TERMINA_FIELD,
																										 ...
																										 RR_SAKON_HIDEOUT })),
			CHECK(RC_ENEMY_DROP_DODONGO,		CanKillEnemy(ACTOR_EN_DODONGO)		&& (CanReachRegions({ RR_TERMINA_FIELD }) && IS_DAY() ||
																										 CanReachRegions({ RR_TERMINA_FIELD_DODONGO_GROTTO }))),
			...
			CHECK(RC_ENEMY_DROP_EENO,			CanKillEnemy(ACTOR_EN_SNOWMAN)		&& (CanReachRegions({ RR_TERMINA_FIELD }) && IS_NIGHT() ||
																										 CanReachRegions({ RR_SNOWHEAD_TEMPLE_SNOW_ROOM }))),
			...
			// TODO: The spawn positions for these vary based on day or night, but until these regions are split, it won't matter.
			CHECK(RC_ENEMY_DROP_LEEVER,			CanKillEnemy(ACTOR_EN_NEO_REEBA)	&& CanReachRegions({ RR_TERMINA_FIELD,
																										 RR_GREAT_BAY_COAST,
																										 RR_ZORA_CAPE })),
			// TODO: Some of these are night only, but won't matter until future region splits.
			...
			CHECK(RC_ENEMY_DROP_MINI_BABA,		CanKillEnemy(ACTOR_EN_KAREBABA)		&& CanReachRegions({ RR_SOUTHERN_SWAMP_NORTH,
																										 ...
																										 RR_ZORA_CAPE_GROTTO })),
			CHECK(RC_ENEMY_DROP_NEJIRON,		CanKillEnemy(ACTOR_EN_BAGUO)		&& (CanReachRegions({ RR_ROAD_TO_IKANA_ABOVE_LEDGE }) && IS_DAY() ||
																										 CanReachRegions({ RR_STONE_TOWER_TEMPLE_MIRROR_PILLAR_ROOM }))),
			...
			CHECK(RC_ENEMY_DROP_PEAHAT,			CanKillEnemy(ACTOR_EN_PEEHAT)		&& CanReachRegions({ RR_TERMINA_FIELD_PEAHAT_GROTTO }) && IS_DAY()),
			...
			CHECK(RC_ENEMY_DROP_STALCHILD,		CanKillEnemy(ACTOR_EN_SKB)			&& CanReachRegions({ RR_IKANA_GRAVEYARD_LOWER,
																										 RR_IKANA_GRAVEYARD_UPPER }) && IS_NIGHT()),
