...
void ApplyOathHint(u16* textId, bool* loadFromMessageTable) {
    ...
    if (Rando::Logic::RemainsCount() < RANDO_SAVE_OPTIONS[RO_ACCESS_MOON_REMAINS_COUNT]) {
        msg = LOCALIZED(
            "You think you can defeat me? The Giants are trapped and powerless to stop me. Even if they were free, they couldn't save you.",
            "Tu crois pouvoir me vaincre? Les Géants sont piégés et impuissants pour même m'arrêter. Même libres,ils ne pourraient pas te sauver.",
        ...
        msg = LOCALIZED(
            "I can hear the Giants Melody ({article}%y{{item}}%w) coming from %y{{location}}%w. But it's too late! They can't help you now!",
            "J'entends la Mélodie des Géants ({article}%y{{item}}%w) venant de %y{{location}}%w. Mais il est trop tard! Ils ne peuvent plus t'aider!",
            "TODO_GERMAN", "TODO_JAPANESE", "TODO_SPANISH");
