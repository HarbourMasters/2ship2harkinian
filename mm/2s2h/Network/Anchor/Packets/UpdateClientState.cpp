...
nlohmann::json Anchor::PrepClientState() {
    ...
    payload["color"] = CVarGetColor24("gNetwork.Anchor.Color", { 100, 255, 100 });
    payload["clientVersion"] = clientVersion;
    payload["teamId"] = CVarGetString("gNetwork.Anchor.TeamId", "default");
    payload["online"] = true;

    if (IsSaveLoaded()) {
        payload["seed"] = gSaveContext.save.shipSaveInfo.rando.finalSeed;
        payload["isSaveLoaded"] = true;
        payload["isGameComplete"] = !!gSaveContext.save.shipSaveInfo.fileCompletedAt;
