...
#include "functions.h"
#include "variables.h"
#include "src/overlays/actors/ovl_Obj_Grass/z_obj_grass.h"
#include "src/overlays/actors/ovl_Obj_Grass_Unit/z_obj_grass_unit.h"
}
...
static std::vector<ActorObject> currentActors;
static std::vector<ObjGrassUnit*> grassUnitActorList;
static std::vector<ObjGrass*> grassActorList;
nlohmann::json ActorObjectToJson(const ActorObject& actor) {
    nlohmann::json jsonObject;
    std::string str = "{ { ";
    str += std::to_string(actor.actorHomePos.x);
    str += ", ";
    str += std::to_string(actor.actorHomePos.z);
    str += " }, ";
...
void ActorViewer_FindGrass() {
    COND_ID_HOOK(OnActorInit, ACTOR_OBJ_GRASS, true, [](Actor* actor) {
        ObjGrass* grassActor = (ObjGrass*)actor;
        grassActorList.push_back(grassActor);
    });
}
void ActorViewer_ClearGrass() {
    grassActorList.clear();
}
void ActorViewer_ExportActors() {
    std::string sceneName = std::to_string(gPlayState->sceneId).c_str();

    nlohmann::json jsonData;
    jsonData[sceneName] = nlohmann::json::object();
    jsonData[sceneName]["Large Snowball"] = nlohmann::json::array();
    jsonData[sceneName]["Small Snowball"] = nlohmann::json::array();

    for (const auto& actor : currentActors) {
        if (!sActorListFilter.PassFilter(actor.actorName.c_str()) &&
            !sActorListFilter.PassFilter(std::to_string(actor.actorId).c_str())) {
            continue;
        }
        if (actor.actorId == ACTOR_OBJ_SNOWBALL) {
            jsonData[sceneName]["Large Snowball"].push_back(ActorObjectToJson(actor));
        } else if (actor.actorId == ACTOR_OBJ_SNOWBALL2) {
            jsonData[sceneName]["Small Snowball"].push_back(ActorObjectToJson(actor));
        } else {
            jsonData[sceneName].push_back(ActorObjectToJson(actor));
        }
    }

    std::ofstream file("Actor_Export.json");
    if (file.is_open()) {
...
void ActorViewer_CollectActors() {
    currentActors.clear();
    auto list = GetCurrentSceneActors();
    for (auto& actors : list) {
        ActorObject tempObject;
        tempObject.actorId = actors->id;
        tempObject.actorName = GetActorDescription(actors->id).c_str();
        tempObject.actorHomePos = actors->home.pos;
        currentActors.push_back(tempObject);
    }
    for (auto& grassList : grassActorList) {
        for (auto& grassGroup : grassList->grassGroups) {
            for (auto& grassActor : grassGroup.elements) {
                if (grassActor.pos.x == 0.0f && grassActor.pos.y == 0.0f && grassActor.pos.z == 0.0f) {
                    continue;
                }
                ActorObject tempObject;
                tempObject.actorId = ACTOR_OBJ_GRASS;
                tempObject.actorName = "Grass";
                tempObject.actorHomePos = grassActor.pos;
                currentActors.push_back(tempObject);
            }
        }
    }
}
void ActorViewer_DrawSceneViewerTab() {
    if (ImGui::Button("Collect Actors")) {
        ActorViewer_CollectActors();
    }
    ImGui::SameLine();
    ...
    if (!sActorListFilter.IsActive()) {
        ImGui::SameLine(18.0f);
        ImGui::Text("Filter Results");
    }

    ImGui::BeginChild("Actor List");
    if (ImGui::BeginTable("Current Actors", 3, ImGuiTableFlags_BordersInnerH | ImGuiTableFlags_ScrollY)) {
        ImGui::TableSetupScrollFreeze(0, 1);
        ImGui::TableSetupColumn("ID", ImGuiTableColumnFlags_WidthFixed, 50.0f);
        ImGui::TableSetupColumn("Name");
        ImGui::TableSetupColumn("Spawn Position");
        ...
        for (auto& actor : currentActors) {
            ...
            ImGui::Text("%d", actor.actorId);
            ImGui::TableNextColumn();
            ImGui::TextWrapped(actor.actorName.c_str());
            ImGui::TableNextColumn();
            ImGui::Text("x: %.2f", actor.actorHomePos.x);
            ImGui::Text("y: %.2f", actor.actorHomePos.y);
            ImGui::Text("z: %.2f", actor.actorHomePos.z);
        }
        ImGui::EndTable();
    }
    ImGui::EndChild();
...
void ActorViewer_DrawMainTab() {
    ...
    if (ImGui::TreeNode("Spawn new Actor")) {
        ...
        if (UIWidgets::Button("Spawn", { .color = UIWidgets::Colors::Green })) {
            Actor_Spawn(&gPlayState->actorCtx, gPlayState, newActor.id, newActor.pos.x, newActor.pos.y, newActor.pos.z,
...
void ActorViewerWindow::DrawElement() {
    if (gPlayState != nullptr) {
        if (ImGui::BeginTabBar("Actor Viewer Tab")) {
            if (ImGui::BeginTabItem("Main")) {
                ActorViewer_DrawMainTab();
                ImGui::EndTabItem();
            }
            if (ImGui::BeginTabItem("Scene Viewer")) {
                ActorViewer_DrawSceneViewerTab();
                ImGui::EndTabItem();
            }
            ImGui::EndTabBar();
        }
