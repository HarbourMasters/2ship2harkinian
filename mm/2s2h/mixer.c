...
void aOPUSdecImplOld(void* source_addr, uint16_t dest_addr, uint16_t nbytes, int32_t* toDecode) {
    if (decState->dec == NULL) {
        decState->dec = opus_decoder_create(48000, 1, NULL);
        decState->overflowPcmData = malloc(0x100000);
        char name[17];
        sprintf(name, "%p", source_addr);
        decState->file = fopen(name, "wb");
    }
    void* h = op_mem_stream_create(NULL, NULL, 0);
    if (pos == 0) {
        decState->opusDataStart = source_addr;
        decState->driverPos = 0;
    }
    int packetSize;

    int totalReadSamples = 0;
    opus_int16 pcm[960 * 6 * 2];
    int dataNeeded = nbytes;
    int dmemOffset = 0;
    memset(BUF_U8(dest_addr), 0, nbytes);
    if (decState->pcmPos > pos) {

        int diff = decState->overflowTotal - decState->overflowPos;
        if (diff > nbytes) {
            diff = nbytes;
        }
        if (diff < 0) {
       ...
       // fwrite((char*)decState->overflowPcmData + decState->overflowPos, 1, diff, decState->file);
        memcpy(BUF_U8(dest_addr), (char*)decState->overflowPcmData + decState->overflowPos, diff);
        dataNeeded -= diff;
        decState->overflowPos += diff;
        dmemOffset += diff;
    }
    if (dataNeeded != 0) {
        decState->overflowPos = 0;
        decState->overflowTotal = 0;
        do {
            memcpy(&packetSize, (char*)decState->opusDataStart +decState->opusPos, 4);
            decState->opusPos += 4;
            void* newDataStart = (char*)decState->opusDataStart + decState->opusPos;
            int readSamples = opus_decode(decState->dec, (char*)decState->opusDataStart + decState->opusPos, packetSize,
                                          pcm, 960 * 6 * 2, 0);
            totalReadSamples += readSamples;
            decState->pcmPos += readSamples;
            decState->opusPos += packetSize;
            if (totalReadSamples * 2 > dataNeeded) {
              //  fwrite(pcm, 1, dataNeeded, decState->file);
                memcpy(BUF_U8(dest_addr + dmemOffset), pcm, dataNeeded);
                break;
            }
            // We read less data than the game is expecting. Copy it into the RSP
            //fwrite(pcm, 1, readSamples * 2, decState->file);
            memcpy(BUF_U8(dest_addr + dmemOffset), pcm, readSamples * 2);
            dmemOffset += readSamples * 2;

        } while (1);
        //fwrite(&pcm, 1, (totalReadSamples * 2), decState->file);
        memcpy((char*)decState->overflowPcmData, pcm, (totalReadSamples * 2));
        decState->overflowPos += dataNeeded;
        decState->overflowTotal = (totalReadSamples * 2);

    }
    fwrite(BUF_U8(dest_addr), nbytes, 1, decState->file);
}
#endif
