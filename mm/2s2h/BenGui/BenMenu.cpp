...
void BenMenu::AddSettings() {
    ...
    AddWidget(path, "Menu Controller Navigation", WIDGET_CVAR_CHECKBOX)
        ...
        .Options(CheckboxOptions().Tooltip("Allows controller navigation of the 2Ship menu (Settings, Enhancements,...)\nCAUTION: "
    ...
    AddWidget(path, "Search In Sidebar", WIDGET_CVAR_CHECKBOX)
        ...
        .Options(CheckboxOptions().Tooltip("Displays the Search menu as a sidebar entry in Settings instead of in the header."));
    AddWidget(path, "Search Input Autofocus", WIDGET_CVAR_CHECKBOX)
        .CVar("gSettings.SearchAutofocus")
        .Options(CheckboxOptions().Tooltip("Search input box gets autofocus when visible. Does not affect using other widgets."));
    AddWidget(path, "Alt Assets Tab hotkey", WIDGET_CVAR_CHECKBOX)
        .CVar("gEnhancements.Mods.AlternateAssetsHotkey")
        .Options(CheckboxOptions().Tooltip("Allows pressing the Tab key to toggle alternate assets").DefaultValue(true));
    ...
    AddWidget(path, "Master Volume: %.0f%%", WIDGET_CVAR_SLIDER_FLOAT)
        ...
        .Options(FloatSliderOptions().Tooltip("Adjust the overall sound volume.")
            .ShowButtons(false).Format("").IsPercentage());
    AddWidget(path, "Main Music Volume: %.0f%%", WIDGET_CVAR_SLIDER_FLOAT)
        ...
        .Options(FloatSliderOptions().Tooltip("Adjust the background music volume.")
            .ShowButtons(false).Format("").IsPercentage());
    AddWidget(path, "Sub Music Volume: %.0f%%", WIDGET_CVAR_SLIDER_FLOAT)
        ...
        .Options(FloatSliderOptions().Tooltip("Adjust the sub music volume.")
            .ShowButtons(false).Format("").IsPercentage());
    AddWidget(path, "Sound Effects Volume: %.0f%%", WIDGET_CVAR_SLIDER_FLOAT)
        ...
        .Options(FloatSliderOptions().Tooltip("Adjust the sound effects volume.")
            .ShowButtons(false).Format("").IsPercentage());
    AddWidget(path, "Fanfare Volume: %.0f%%", WIDGET_CVAR_SLIDER_FLOAT)
        ...
        .Options(FloatSliderOptions().Tooltip("Adjust the fanfare volume.")
            .ShowButtons(false).Format("").IsPercentage());
    AddWidget(path, "Ambience Volume: %.0f%%", WIDGET_CVAR_SLIDER_FLOAT)
        ...
        .Options(FloatSliderOptions().Tooltip("Adjust the ambient sound volume.")
            .ShowButtons(false).Format("").IsPercentage());
    ...
    AddWidget(path, "Internal Resolution: %.0f%%", WIDGET_CVAR_SLIDER_FLOAT)
        ...
        .Options(FloatSliderOptions()
            ...
            .ShowButtons(false).IsPercentage().Format("").Min(0.5f).Max(2.0f));
#endif
#ifndef __WIIU__
    AddWidget(path, "Anti-aliasing (MSAA): %d", WIDGET_CVAR_SLIDER_INT)
        ...
        .Options(IntSliderOptions()
            .Tooltip("Activates MSAA (multi-sample anti-aliasing) from 2x up to 8x, to smooth the edges of rendered geometry.\n"
    ...
    AddWidget(path, "Current FPS: %d", WIDGET_CVAR_SLIDER_INT)
        ...
        .Options(IntSliderOptions().Tooltip(tooltip)
            .Min(20).Max(maxFps).DefaultValue(20));
    ...
    AddWidget(path, "Allow multi-windows", WIDGET_CVAR_CHECKBOX)
        ...
        .Options(CheckboxOptions().Tooltip("Allows multiple windows to be opened at once. Requires a reload to take effect."));
    ...
    AddWidget(path, "Position", WIDGET_CVAR_COMBOBOX)
        ...
        .Options(
            ComboboxOptions().Tooltip("Which corner of the screen notifications appear in.")
            .ComboMap(notificationPosition)
            .DefaultIndex(3));
    AddWidget(path, "Duration: %.0f seconds", WIDGET_CVAR_SLIDER_FLOAT)
        .CVar("gNotifications.Duration")
        .Options(
            FloatSliderOptions().Tooltip("How long notifications are displayed for.")
    ...
    AddWidget(path, "Background Opacity: %.0f%%", WIDGET_CVAR_SLIDER_FLOAT)
        ...
        .Options(
            FloatSliderOptions().Tooltip("How opaque the background of notifications is.")
            .DefaultValue(0.5f).IsPercentage());
    AddWidget(path, "Size %.1f", WIDGET_CVAR_SLIDER_FLOAT)
        .CVar("gNotifications.Size")
        .Options(
            FloatSliderOptions().Tooltip("How large notifications are.")
...
void BenMenu::AddEnhancements() {
    ...
    AddWidget(path, "Fix Targeting Camera Snap", WIDGET_CVAR_CHECKBOX)
        ...
        .Options(CheckboxOptions().Tooltip("Fixes the camera snap that occurs when you are moving and press the targeting button."));
    ...
    AddWidget(path, "Invert Y Axis", WIDGET_CVAR_CHECKBOX)
        ...
        .Options(CheckboxOptions().Tooltip("Inverts the Y Axis of the Camera in First Person Mode.")
            .DefaultValue(true));
    AddWidget(path, "X Axis Sensitivity: %.0f%%", WIDGET_CVAR_SLIDER_FLOAT)
        .CVar("gEnhancements.Camera.FirstPerson.SensitivityX")
        .Options(FloatSliderOptions().Tooltip("Adjusts the Sensitivity of the X Axis in First Person Mode.")
            .Min(0.01f).Max(2.0f).DefaultValue(1.0f).IsPercentage());
    AddWidget(path, "Y Axis Sensitivity: %.0f%%", WIDGET_CVAR_SLIDER_FLOAT)
        .CVar("gEnhancements.Camera.FirstPerson.SensitivityY")
        .Options(FloatSliderOptions().Tooltip("Adjusts the Sensitivity of the Y Axis in First Person Mode.")
            .Min(0.01f).Max(2.0f).DefaultValue(1.0f).IsPercentage());
    ...
    AddWidget(path, "Gyro X Axis Sensitivity: %.0f%%", WIDGET_CVAR_SLIDER_FLOAT)
        ...
        .Options(FloatSliderOptions().Tooltip("Adjusts the Sensitivity of the X Axis of the Gyro in First Person Mode.")
            .Min(0.01f).Max(2.0f).DefaultValue(1.0f).IsPercentage())
        .PreFunc([](WidgetInfo& info) { info.isHidden = mBenMenu->disabledMap.at(DISABLE_FOR_GYRO_OFF).active; });
    AddWidget(path, "Gyro Y Axis Sensitivity: %.0f%%", WIDGET_CVAR_SLIDER_FLOAT)
        .CVar("gEnhancements.Camera.FirstPerson.GyroSensitivityY")
        .Options(FloatSliderOptions().Tooltip("Adjusts the Sensitivity of the Y Axis of the Gyro in First Person Mode.")
            .Min(0.01f).Max(2.0f).DefaultValue(1.0f).IsPercentage())
        .PreFunc([](WidgetInfo& info) { info.isHidden = mBenMenu->disabledMap.at(DISABLE_FOR_GYRO_OFF).active; });
    ...
    AddWidget(path, "Invert Right Stick Y Axis", WIDGET_CVAR_CHECKBOX)
        ...
        .PreFunc(
            [](WidgetInfo& info) { info.isHidden = mBenMenu->disabledMap.at(DISABLE_FOR_RIGHT_STICK_OFF).active; })
        .Options(CheckboxOptions().Tooltip("Inverts the Y Axis of the Right Stick in First Person Mode.")
            .DefaultValue(true));
    AddWidget(path, "Right Stick X Axis Sensitivity: %.0f%%", WIDGET_CVAR_SLIDER_FLOAT)
        ...
        .PreFunc(
            [](WidgetInfo& info) { info.isHidden = mBenMenu->disabledMap.at(DISABLE_FOR_RIGHT_STICK_OFF).active; })
        .Options(FloatSliderOptions().Tooltip("Adjusts the Sensitivity of the X Axis of the Right Stick in First Person Mode.")
            .Min(0.01f).Max(2.0f).DefaultValue(1.0f).IsPercentage());
    AddWidget(path, "Right Stick Y Axis Sensitivity: %.0f%%", WIDGET_CVAR_SLIDER_FLOAT)
        ...
        .PreFunc(
            [](WidgetInfo& info) { info.isHidden = mBenMenu->disabledMap.at(DISABLE_FOR_RIGHT_STICK_OFF).active; })
        .Options(FloatSliderOptions().Tooltip("Adjusts the Sensitivity of the Y Axis of the Right Stick in First Person Mode.")
            .Min(0.01f).Max(2.0f).DefaultValue(1.0f).IsPercentage());
    ...
    AddWidget(path, "Free Look", WIDGET_CVAR_CHECKBOX)
        ...
        .Options(CheckboxOptions().Tooltip("Enables free look camera control\nNote: You must remap C buttons off of the right "
    ...
    AddWidget(path, "Camera Distance: %d", WIDGET_CVAR_SLIDER_INT)
        ...
        .Options(IntSliderOptions().Tooltip("Maximum Camera Distance for Free Look.")
    ...
    AddWidget(path, "Max Camera Height Angle: %.0f\xC2\xB0", WIDGET_CVAR_SLIDER_FLOAT)
        ...
        .Options(FloatSliderOptions().Tooltip("Maximum Height of the Camera.")
    ...
    AddWidget(path, "Min Camera Height Angle: %.0f\xC2\xB0", WIDGET_CVAR_SLIDER_FLOAT)
        ...
        .Callback([](WidgetInfo& info) { FreeLookPitchMinMax(); })
        .PreFunc([](WidgetInfo& info) { info.isHidden = mBenMenu->disabledMap.at(DISABLE_FOR_FREE_LOOK_OFF).active; })
        .Options(FloatSliderOptions().Tooltip("Minimum Height of the Camera.")
    ...
    AddWidget(path, "Third-Person Horizontal Sensitivity: %.0f", WIDGET_CVAR_SLIDER_FLOAT)
        ...
        .Options(FloatSliderOptions().Tooltip("Adjust the Sensitivity of the x axis when in Third Person.")
    ...
    AddWidget(path, "Enable Roll (6\xC2\xB0 of Freedom)", WIDGET_CVAR_CHECKBOX)
        ...
        .Options(CheckboxOptions().Tooltip("This allows for all six degrees of movement with the camera, NOTE: Yaw will work "
    ...
    AddWidget(path, "Camera Speed: %.0f", WIDGET_CVAR_SLIDER_FLOAT)
        ...
        .PreFunc([](WidgetInfo& info) { info.isHidden = mBenMenu->disabledMap.at(DISABLE_FOR_DEBUG_CAM_OFF).active; })
        .Options(FloatSliderOptions().Tooltip("Adjusts the speed of the Camera.")
            .Format("%.0f%%").Min(0.1f).Max(3.0f).DefaultValue(0.5f).IsPercentage());
    ...
    AddWidget(path, "Climb speed", WIDGET_CVAR_SLIDER_INT)
        ...
        .Options(IntSliderOptions().Tooltip("Increases the speed at which Link climbs vines and ladders.")
    ...
    AddWidget(path, "Instant Fin Boomerangs Recall", WIDGET_CVAR_CHECKBOX)
        ...
        .Options(CheckboxOptions().Tooltip("Pressing B will instantly recall the fin boomerang back to Zora Link after they are thrown."));
    ...
    AddWidget(path, "Hyrule Warriors Styled Link", WIDGET_CVAR_CHECKBOX)
        ...
        .Options(CheckboxOptions().Tooltip("When acquired, places the Keaton and Fierce Deity masks on Link similarly to how he "
    ...
    AddWidget(path, "Pause Menu Save", WIDGET_CVAR_CHECKBOX)
        ...
        .Options(CheckboxOptions().Tooltip("Re-introduce the pause menu save system. Pressing B in the pause menu will give you the "
    ...
    AddWidget(path, "Autosave", WIDGET_CVAR_CHECKBOX)
        ...
        .Options(CheckboxOptions().Tooltip("Automatically create a persistent Owl Save on the chosen interval.\n\nWhen loading "
    ...
    AddWidget(path, "Do not reset Time Speed", WIDGET_CVAR_CHECKBOX)
        ...
        .Options(CheckboxOptions().Tooltip("Playing the Song Of Time will not reset the current time speed set by Inverted Song of Time."));
    AddWidget(path, "Keep Express Mail", WIDGET_CVAR_CHECKBOX)
        ...
        .Options(CheckboxOptions().Tooltip("Allows the player to keep the Express Mail in their inventory after delivering it "
    ...
    AddWidget(path, "Disable Save Delay", WIDGET_CVAR_CHECKBOX)
        ...
        .Options(CheckboxOptions().Tooltip("Removes the arbitrary 2 second timer for saving from the original game. This is known to "
    ...
    AddWidget(path, "Clock Type", WIDGET_CVAR_COMBOBOX)
        ...
        .Options(ComboboxOptions().Tooltip("Swaps between Graphical and Text only Clock types.")
    ...
    AddWidget(path, "Use Alternate Assets", WIDGET_CVAR_CHECKBOX)
        ...
        .Options(CheckboxOptions().Tooltip("Toggle between standard assets and alternate assets. Usually mods will indicate if "
    ...
    AddWidget(path, "Motion Blur Mode", WIDGET_CVAR_COMBOBOX)
        ...
        .Options(ComboboxOptions().Tooltip("Selects the Mode for Motion Blur.").LabelPosition(LabelPosition::None)
    ...
    AddWidget(path, "Interpolate", WIDGET_CVAR_CHECKBOX)
        ...
        .Options(CheckboxOptions().Tooltip("Change motion blur capture to also happen on interpolated frames instead of only on game frames.\n"
    ...
    AddWidget(path, "Disable Black Bar Letterboxes", WIDGET_CVAR_CHECKBOX)
        ...
        .Options(CheckboxOptions().Tooltip("Disables Black Bar Letterboxes during cutscenes and Z-targeting\nNote: there may be "
    ...
    AddWidget(path, "Disable Scene Geometry Distance Check", WIDGET_CVAR_CHECKBOX)
        ...
        .Options(CheckboxOptions().Tooltip("Disables the distance check for scene geometry, allowing it to be drawn no matter how far "
    ...
    AddWidget(path, "Increase Actor Draw Distance: %dx", WIDGET_CVAR_SLIDER_INT)
        ...
        .Options(IntSliderOptions().Tooltip("Increase the range in which Actors are drawn. This may have unintended side effects.")
            .Min(1).Max(5).DefaultValue(1));
    ...
    AddWidget(path, "Persistent Bunny Hood", WIDGET_CVAR_CHECKBOX)
        ...
        .Options(CheckboxOptions().Tooltip("Permanently toggle a speed boost from the bunny hood by pressing 'A' on it in the mask menu."));
    ...
    AddWidget(path, "Better Song of Double Time", WIDGET_CVAR_CHECKBOX)
        ...
        .Options(CheckboxOptions().Tooltip("When playing the Song of Double Time, you can now choose the exact time you want to go "
    ...
    AddWidget(path, "Enable Sun's Song", WIDGET_CVAR_CHECKBOX)
        ...
        .Options(CheckboxOptions().Tooltip("Enables the partially implemented Sun's Song. RIGHT-DOWN-UP-RIGHT-DOWN-UP to play it. "
    ...
    AddWidget(path, "Pause Owl Warp", WIDGET_CVAR_CHECKBOX)
        ...
        .Options(CheckboxOptions().Tooltip("Allows warping to registered Owl Statues from the pause menu map screen. "
    ...
    AddWidget(path, "Zora Eggs For Bossa Nova", WIDGET_CVAR_SLIDER_INT)
        ...
        .Options(IntSliderOptions().Tooltip("The number of eggs required to unlock new wave bossa nova.")
    ...
    AddWidget(path, "Skip to File Select", WIDGET_CVAR_CHECKBOX)
        ...
        .Options(CheckboxOptions().Tooltip("Skip the opening title sequence and go straight to the file select menu after boot."));
    AddWidget(path, "Skip Intro Sequence", WIDGET_CVAR_CHECKBOX)
        .CVar("gEnhancements.Cutscenes.SkipIntroSequence")
        .Options(CheckboxOptions().Tooltip("When starting a game you will be taken straight to South Clock Town as Deku Link."));
    AddWidget(path, "Skip First Cycle", WIDGET_CVAR_CHECKBOX)
        ...
        .Options(CheckboxOptions().Tooltip("When starting a game you will be taken straight to South Clock Town as Human Link "
    ...
    AddWidget(path, "Skip Story Cutscenes", WIDGET_CVAR_CHECKBOX)
        ...
        .Options(CheckboxOptions().Tooltip("Disclaimer: This doesn't do much yet, we will be progressively adding more skips over time."));
    ...
    AddWidget(path, "Fast Bank Selection", WIDGET_CVAR_CHECKBOX)
        ...
        .Options(CheckboxOptions().Tooltip("Pressing the Z or R buttons while the Deposit/Withdrawal Rupees dialogue is open will set "
    ...
    AddWidget(path, "Fix Ikana Great Fairy Fountain Color", WIDGET_CVAR_CHECKBOX)
        ...
        .Options(CheckboxOptions().Tooltip("Fixes a bug that results in the Ikana Great Fairy fountain looking green instead of "
    ...
    AddWidget(path, "Fix Completed Heart Container Audio", WIDGET_CVAR_CHECKBOX)
        ...
        .Options(CheckboxOptions().Tooltip("Fixes a bug that results in the wrong audio playing upon receiving a 4th piece of heart to "
    ...
    AddWidget(path, "Power Crouch Stab", WIDGET_CVAR_CHECKBOX)
        ...
        .Options(CheckboxOptions().Tooltip("Crouch stabs will use the power of Link's previous melee attack, as is in MM JP 1.0 and OoT."));
    ...
    AddWidget(path, "Disable Takkuri Steal", WIDGET_CVAR_CHECKBOX)
        ...
        .Options(CheckboxOptions().Tooltip("Prevents the Takkuri from stealing key items like bottles and swords. It may still steal "
                 "other items."));
    ...
    AddWidget(path, "Popout HUD Editor", WIDGET_WINDOW_BUTTON)
        ...
        .Options(ButtonOptions().Tooltip("Enables the HUD Editor window, allowing you to modify your HUD")
            .Size(Sizes::Inline ));
...
void BenMenu::AddDevTools() {
    ...
    AddWidget(path, "Better Map Select", WIDGET_CVAR_CHECKBOX)
        ...
        .Options(CheckboxOptions().Tooltip("Overrides the original map select with a translated, more user-friendly version."))
    ...
    AddWidget(path, "Debug Save File Mode", WIDGET_CVAR_COMBOBOX)
        ...
        .Options(ComboboxOptions().Tooltip("Change the behavior of creating saves while debug mode is enabled:\n\n"
    ...
    AddWidget(path, "Log Level", WIDGET_CVAR_COMBOBOX)
        ...
        .Options(ComboboxOptions().Tooltip("The log level determines which messages are printed to the "
    ...
    AddWidget(path, "Frame Advance", WIDGET_CHECKBOX)
        .Options(CheckboxOptions().Tooltip("This allows you to advance through the game one frame at a time on command. "
    ...
    AddWidget(path, "Popout Console", WIDGET_WINDOW_BUTTON)
        ...
        .Options(ButtonOptions().Tooltip("Enables the console window, allowing you to input commands. Type help for some examples"))
        .WindowName("Console");
    ...
    AddWidget(path, "Popout Gfx Debugger", WIDGET_WINDOW_BUTTON)
        ...
        .Options(ButtonOptions().Tooltip("Enables the Gfx Debugger window, allowing you to input commands, type help for some examples"))
