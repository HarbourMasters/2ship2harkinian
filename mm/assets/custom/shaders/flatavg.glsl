@prism(type='fragment', name='Flat Average Colour', version='1.0.0', description='Ported shader to prism', author='Emill & Prism Team')
...
@else
    ...
    vec4 avgTexture2D(sampler2D tex) {
        vec4 sum = vec4(0.0);
        for (int y = 0; y < 8; y++) {
            for (int x = 0; x < 8; x++) {
                sum += @{texture}(tex, vec2((float(x) + 0.5) / 8.0, (float(y) + 0.5) / 8.0));
            }
        }
        return sum / 64.0;
    }
    ...
    void main() {
        @for(i in 0..2)
            @if(o_textures[i])
                ...
                texVal@{i}.rgb = avgTexture2D(uTex@{i}).rgb;
