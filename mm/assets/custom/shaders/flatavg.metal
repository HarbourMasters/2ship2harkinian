@prism(type='metal', name='Flat Average Colour', version='1.0.0', description='Ported shader to prism', author='Emill & Prism Team')
...
float4 avgTexture2D(thread const texture2d<float> tex, thread const sampler texSmplr) {
    float4 sum = float4(0.0);
    for (int y = 0; y < 8; y++) {
        for (int x = 0; x < 8; x++) {
            sum += tex.sample(texSmplr, float2((float(x) + 0.5) / 8.0, (float(y) + 0.5) / 8.0));
        }
    }
    return sum / 64.0;
}
...
@end
    ...
    @for(i in 0..2)
        @if(o_textures[i])
            ...
            texVal@{i}.rgb = avgTexture2D(uTex@{i}, uTex@{i}Smplr).rgb;
