@prism(type='hlsl', name='Flat Average Colour', version='1.0.0', description='Ported shader to prism', author='Emill & Prism Team')
...
float4 avgTexture2D(Texture2D tex, SamplerState smp) {
    float4 sum = float4(0.0, 0.0, 0.0, 0.0);
    for (int y = 0; y < 8; y++) {
        for (int x = 0; x < 8; x++) {
            sum += tex.SampleLevel(smp, float2((x + 0.5) / 8.0, (y + 0.5) / 8.0), 0);
        }
    }
    return sum / 64.0;
}
            ...
            @if(o_three_point_filtering)
                ...
                if (textures[@{i}].linear_filtering) {
                    @if(o_masks[i])
                        ...
                        texVal@{i}.rgb = avgTexture2D(g_texture@{i}, g_sampler@{i}).rgb;
                        float2 maskSize@{i};
