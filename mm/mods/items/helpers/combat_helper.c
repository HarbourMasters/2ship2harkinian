...
void Combat_UpdateCylinder(ColliderCylinder* col, Vec3f* pos, CombatColliderConfig* cfg) {
    ...
    col->elem.atDmgInfo.dmgFlags = cfg->dmgFlags;
    col->elem.atDmgInfo.damage = cfg->damage;
    col->elem.atDmgInfo.effect = cfg->effect;
...
u8 Combat_IsActorInRange(Actor* actor, f32 xzRange, f32 yRange) {
    if (actor == NULL)
        return 0;
    return (actor->xzDistToPlayer < xzRange) && (fabsf(actor->playerHeightRel) < yRange);
}
