...
#include "mods/oot_asset_loader/oot_asset_loader.h" // OotAssets_LoadGfx (real OoT DLs from oot.o2r)

// Lazily resolve OoT's gEffFireCircleDL (the vertical fire-ring used by SoH for the rod charge
// aura + spin cylinder). MM's gameplay_keep has no equivalent, so the earlier port substituted the
// flat gEffShockwaveDL quad — which is why the aura/cylinder were invisible. When oot.o2r is mounted
// this returns the true SoH geometry for a 1:1 look; NULL otherwise (callers fall back to the MM
// gEffShockwaveDL placeholder, upscaled so at least *something* shows). Cached. Skijer's NEI
static Gfx* sFireCircleDL = NULL;
static u8 sFireCircleTried = 0;

static Gfx* FX_GetFireCircleDL(void) {
    if (!sFireCircleTried) {
        sFireCircleTried = 1;
        sFireCircleDL = (Gfx*)OotAssets_LoadGfx("__OTR__objects/gameplay_keep/gEffFireCircleDL");
    }
    return sFireCircleDL;
}
...
void FX_SpawnTrail(PlayState* play, Vec3f* pos, FX_Type type) {
    Vec3f zero = { 0, 0, 0 };
    if (type == FX_FIRE)
        EffectSsDFire_Spawn(play, pos, &zero, &zero, 15, 1, 255, 80, 20, 8);
    else if (type == FX_ICE)
...
void FX_DrawChargeAura(PlayState* play, Player* player, f32 chargeLevel, RodColor* color) {
    ...
    // 1:1 with SoH: draw OoT's gEffFireCircleDL when oot.o2r is mounted (same segment-8 texscroll,
    // colors and scale as above are copied verbatim from SoH's FX_DrawChargeAura). Fallback: MM's
    // flat gEffShockwaveDL quad, which needs a much larger scale than OoT's tiny fire-ring geometry.
    Gfx* fireCircle = FX_GetFireCircleDL();
    f32 fallbackMul = (fireCircle != NULL) ? 1.0f : 25.0f;

    Matrix_Translate(pos.x, pos.y, pos.z, MTXMODE_NEW);
    Matrix_RotateY(player->actor.shape.rot.y * (M_PI / 0x8000), MTXMODE_APPLY);
    Matrix_Scale(scaleXZ * fallbackMul, scaleY * fallbackMul, scaleXZ * fallbackMul, MTXMODE_APPLY);

    gSPMatrix(POLY_XLU_DISP++, MATRIX_NEWMTX(play->state.gfxCtx), G_MTX_NOPUSH | G_MTX_LOAD | G_MTX_MODELVIEW);
    gSPDisplayList(POLY_XLU_DISP++, fireCircle != NULL ? fireCircle : (Gfx*)gEffShockwaveDL);
...
void FX_DrawSpinFireCylinder(PlayState* play, Player* player, f32 radius, u8 isBigSpin, RodColor* color) {
    ...
    // 1:1 with SoH: OoT gEffFireCircleDL when available (scale/segment-8 copied from SoH's
    // FX_DrawSpinFireCylinder); else MM gEffShockwaveDL upscaled. See FX_DrawChargeAura. NEI
    Gfx* fireCircle = FX_GetFireCircleDL();
    f32 fallbackMul = (fireCircle != NULL) ? 1.0f : 25.0f;
