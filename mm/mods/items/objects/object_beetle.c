...
// Beetle model from the o2r (object_nei_beetle) — NOT a compiled DL (all NEI models load
// from the archive; compiled model.inc.c data crashed Fast3D).
static Gfx* Beetle_GetDL(const char* otr, Gfx** cache, u8* tried) {
    if (!*tried) {
        *tried = 1;
...
static Gfx* Beetle_GetBodyDL(void) {
    static Gfx* sDL = NULL;
    static u8 sTried = 0;
    return Beetle_GetDL("__OTR__objects/object_nei_beetle/g_beetle_body_dl", &sDL, &sTried);
}
static Gfx* Beetle_GetWingsDL(void) {
    static Gfx* sDL = NULL;
    static u8 sTried = 0;
    return Beetle_GetDL("__OTR__objects/object_nei_beetle/g_beetle_wings_dl", &sDL, &sTried);
}
#include "macros.h"
#include "functions.h"
...
static void Beetle_DrawWings(PlayState* play, f32 wingScale) {
    ...
    Matrix_Scale(1.0f, wingScale, 1.0f, MTXMODE_APPLY);
    {
        ...
        if (wingsDL != NULL) {
            gSPMatrix(POLY_OPA_DISP++, MATRIX_NEWMTX(play->state.gfxCtx), G_MTX_NOPUSH | G_MTX_LOAD | G_MTX_MODELVIEW);
            gSPDisplayList(POLY_OPA_DISP++, wingsDL);
        }
    }
    Matrix_Pop();
