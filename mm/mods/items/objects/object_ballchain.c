...
// Ball model is loaded from the o2r (object_nei_ball_and_chain) — NOT a compiled DL. See
// BallChain_GetBallDL below. (Compiled model.inc.c caused crashes; all NEI models use the o2r.)

// OoT's gLinkAdultHookshotChainDL isn't a linked MM symbol; load it via ResourceMgr
// (path registered by pak_loader). Chain render is skipped if the archive is absent.
...
// Ball DL from the o2r (packed from mm/assets/custom/objects/object_nei_ball_and_chain). Cached;
// skipped if the archive lacks it (no crash) — the pattern every NEI item model uses.
static Gfx* BallChain_GetBallDL(void) {
    static Gfx* sDL = NULL;
    static u8 sTried = 0;
    if (!sTried) {
        sTried = 1;
        const char* otr = "__OTR__objects/object_nei_ball_and_chain/gBallDL";
...
static void BallChain_DrawBall(PlayState* play, Vec3f* pos, f32 scale, u8 shouldRotate) {
...

    {
        ...
        if (ballDL != NULL) {
            ...
            gSPDisplayList(POLY_OPA_DISP++, ballDL);
        }
    }
...
static void BallChain_DrawChain(PlayState* play, Vec3f* start, Vec3f* end) {
    ...
    static Gfx* sChainDL = NULL;
    static u8 sChainTried = 0;
    if (!sChainTried) {
        sChainTried = 1;
        if (ResourceMgr_FileExists("__OTR__objects/object_link_boy/gLinkAdultHookshotChainDL"))
            sChainDL = ResourceMgr_LoadGfxByName("__OTR__objects/object_link_boy/gLinkAdultHookshotChainDL");
    }
    if (sChainDL == NULL)
        return; // chain asset unavailable → skip chain render
    ...
    for (i = 0; i <= linkCount; i++) {
        ...
        gSPMatrix(POLY_OPA_DISP++, Matrix_NewMtx(play->state.gfxCtx, __FILE__, __LINE__),
                  G_MTX_NOPUSH | G_MTX_LOAD | G_MTX_MODELVIEW);
        gSPDisplayList(POLY_OPA_DISP++, sChainDL);
    }
