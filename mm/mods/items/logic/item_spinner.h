...
// Breakable rock actor IDs. These MUST be MM ids (the old 0x14E/0x1D2/0x127/0x130 were OoT ids that
// in MM resolve to the Inn bell / a rupee / unset / the Great Fairy — so NOTHING broke). They match
// the DestroyBreakable() switch below. Skijer's NEI
static const s16 sBreakableRockIds[] = {
    0x00B0, // ACTOR_EN_ISHI       — liftable rocks / silver boulders
    0x00FC, // ACTOR_OBJ_HAMISHI   — bronze boulder (the Powder Keg / Goron rock)
    0x0092, // ACTOR_OBJ_BOMBIWA   — bombable boulder
    0x0099, // ACTOR_EN_GOROIWA    — rolling boulder
};
...
static ColliderCylinderInit sSpinnerColInit = {
    ...
    { ELEM_MATERIAL_UNK2,
      { DMG_SWORD, 0x00, SPINNER_DMG_RIDE },
      { 0, 0, 0 },
