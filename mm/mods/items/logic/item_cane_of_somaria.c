...
// Include animation
#include "../anim/nei_anims.h" // cast animation now loads from 2ship.o2r — Skijer's NEI
...
static void Somaria_SpawnBlock(Player* p, PlayState* play) {
    ...
    f32 floorHeight = BgCheck_EntityRaycastFloor5(&play->colCtx, &outPoly, &bgId, &p->actor, &spawnPos);
...
// Animation uses skelAnimeUpper with the somaria cast anim from 2ship.o2r (60 frames)
...
static void Somaria_StartCastAnim(Player* p, PlayState* play) {
    // Play the somaria casting animation (loaded from 2ship.o2r). The block spawn is driven by this
