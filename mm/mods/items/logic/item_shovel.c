...
#include "../anim/nei_anims.h" // dig animation now loads from 2ship.o2r — Skijer's NEI

// (Item_DropCollectible is declared by MM's functions.h — the OoT-signature extern was dropped)
...
static void ActivateSoftSoil(PlayState* play, Actor* soilActor) {
    (void)soilActor; // (MM ObjMakekinsuta has no unk_152 — soft-soil flag set dropped. TODO adapt.)
...
static void RevealGrotto(PlayState* play, Actor* grottoActor) {
    ...
    if ((grotto->actor.params & 0x300) != 0) {
        if ((grotto->actor.params & 0x200) != 0) {
            // (MM DoorAna has no collider field — collider destroy dropped. TODO adapt.)
        }
...
static void Shovel_Start(Player* p, PlayState* play) {
    // Dig animation is loaded from 2ship.o2r. The dig is driven by this animation's progress, so if
    // the resource is missing we must not enter the animating state at all. Skijer's NEI
...
// ============================================================================
// UPPER ACTION - Drives the dig animation via skelAnimeUpper
// ============================================================================
