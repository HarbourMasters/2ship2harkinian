...
#include "../anim/nei_anims.h" // animation now loads from 2ship.o2r — Skijer's NEI
...
static void Demise_StateWindup(Player* p, PlayState* play) {
    ...
    if (ddTimer == 0) {
        // Loaded from 2ship.o2r; skip the anim change if the resource is missing (the timer-driven
    ...
    if (ddTimer > 20) {
        if ((ddTimer - 20) % 8 == 0)
            FX_SpawnLightningRing(play, &p->actor.world.pos, 200.0f, 560.0f, 1, 1, 80);
...
static void Demise_StateFinish(Player* p, PlayState* play) {
    ...
    ddCollider.elem.atDmgInfo.dmgFlags = DEMISE_DAMAGE_FLAGS;
    ddCollider.elem.atDmgInfo.damage = DEMISE_DAMAGE;
    ddCollider.elem.atDmgInfo.effect = 1;
