/*
 * mm_sfx_synth_seqplayer.cpp
 *
 * Near-verbatim port of Majora's Mask (2ship2harkinian) audio sequence player,
 * mm/src/audio/lib/seqplayer.c (audio_seqplayer.c), isolated for the SoH SFX
 * translator. Everything lives in `namespace mmsfx`. Every `gAudioCtx.` was
 * rewritten to `gMmSfx.`. MM function names are kept VERBATIM (they all live in
 * namespace mmsfx, so no collision with SoH symbols). The 2S2H AudioEditor /
 * custom-sequence / ResourceMgr hooks (opcodes 0xEB, 0xC6) were STRIPPED back to
 * the vanilla MM path.
 *
 * ===========================================================================
 * (a) EXTERNAL FUNCTIONS CALLED — defined in OTHER MM lib files (playback.c,
 *     heap.c, load.c, effects.c, or this module's own ProcessSound). NOT
 *     defined here; the glue must provide them, all inside namespace mmsfx:
 * ---------------------------------------------------------------------------
 *   // playback.c
 ...
 *   void  AudioScript_SequencePlayerProcessSound(SequencePlayer* seqPlayer); // (defined in effects.c in MM)
 *
 *   // heap.c
 *   void* AudioHeap_AllocZeroed(AudioAllocPool* pool, u32 size);
 *   void* AudioHeap_SearchCaches(s32 tableType, s32 cache, s32 id); // FONT_TABLE, CACHE_EITHER
 ...
 *
 *   // thread (audio_thread.c)
 *   u32   AudioThread_NextRandom(void);
 *
 * ===========================================================================
 * (b) CONTEXT FIELDS USED — gMmSfx.<field> with its MM type (from
 *     AudioContext in mm/include/z64audio.h). Build the isolated context to
 *     match these (and the embedded AudioBufferParameters / AudioCache):
 * ---------------------------------------------------------------------------
 *   SequencePlayer        seqPlayers[5];
 *   SequenceLayer         sequenceLayers[80];
 *   SequenceChannel       sequenceChannelNone;
 *   AudioListItem         layerFreeList;
 *   SynthesisReverb       synthesisReverbs[4];   // .tunedSample (TunedSample) is read
 *   AudioBufferParameters audioBufferParameters; // .updatesPerFrame (s16), .numSequencePlayers (s16)
 *   f32                   unk_2870;
 *   s32                   numNotes;
 *   s16                   maxTempo;
 *   u32                   audioRandom;
 *   u32 (*customSeqFunctions[4])(s8 value, SequenceChannel* channel); // AudioCustomSeqFunction
 *   AudioAllocPool        miscPool;
 *   AudioCache            fontCache;             // .temporary.entries[0..1].id (u16/u8), .temporary.nextSide (u32)
 *   s32                   sampleStateOffset;
 *
 * ===========================================================================
 * (c) DATA TABLES referenced — live in MM's data.c (provide as externs inside
 *     namespace mmsfx):
 * ---------------------------------------------------------------------------
 *   extern EnvelopePoint gDefaultEnvelope[];
 *   extern f32 gBendPitchOneOctaveFrequencies[];   // [256]
 *   extern f32 gBendPitchTwoSemitonesFrequencies[];// [256]
 *   extern f32 gPitchFrequencies[];                // [256]
 *   extern u8  gDefaultShortNoteVelocityTable[];   // [16]
 *   extern u8  gDefaultShortNoteGateTimeTable[];   // [16]
 *   extern AudioCustomSeqFunction gAudioCustomSeqFunction; // global function-ptr scratch (0xBE)
 *
 *   Also requires these enum/macro values (z64audio.h / load.h):
 *     SEQPLAYER_STATE_0=0, SEQPLAYER_STATE_FADE_IN=1, SEQPLAYER_STATE_FADE_OUT=2
 *     FONT_TABLE, CACHE_EITHER
 *     LOAD_STATUS_COMPLETE, LOAD_STATUS_DISCARDABLE, LOAD_STATUS_MAYBE_DISCARDABLE
 *     MUTE_FLAGS_* , TATUMS_PER_BEAT, SEQ_NUM_CHANNELS, SEQ_IO_VAL_NONE
 * ===========================================================================
 */

#include "mm_sfx_synth_ctx.h" // brings in types.h, the full AudioContext, gMmSfx
#include <cstdio>

namespace mmsfx {

// ---- Constants / enums / macros (verbatim from MM z64audio.h, load.h) -----
...
enum { CACHE_TEMPORARY, CACHE_PERSISTENT, CACHE_EITHER, CACHE_PERMANENT };
enum {
    LOAD_STATUS_NOT_LOADED,
    LOAD_STATUS_IN_PROGRESS,
    LOAD_STATUS_COMPLETE,
    LOAD_STATUS_DISCARDABLE,
    LOAD_STATUS_MAYBE_DISCARDABLE,
    LOAD_STATUS_PERMANENTLY_LOADED
};
...
// Big-endian 16-bit swap (replaces 2S2H's BE16SWAP from ship/binarytools).
static inline u16 MMSFX_BE16SWAP(u16 x) {
    return (u16)((x << 8) | (x >> 8));
}
#define BE16SWAP(x) MMSFX_BE16SWAP((u16)(x))

// ---- External data tables (MM data.c) -------------------------------------
extern EnvelopePoint gDefaultEnvelope[];
...
extern "C" void MmSfxSynth_Log(const char* msg); // diagnostics

// ---- External functions (other MM lib TUs; glue provides) -----------------
void AudioPlayback_InitNoteLists(NotePool* pool);
void AudioPlayback_NotePoolClear(NotePool* pool);
void AudioPlayback_NotePoolFill(NotePool* pool, s32 count);
void AudioPlayback_SeqLayerNoteDecay(SequenceLayer* layer);
 ...
 * as the type of each argument. Bitpacked as abcUUUnn.
 */
...
void AudioScript_InitSequenceChannel(SequenceChannel* channel) {
    ...
    if (channel == &gMmSfx.sequenceChannelNone) {
        return;
    }
...
s32 AudioScript_SeqChannelSetLayer(SequenceChannel* channel, s32 layerIndex) {
    ...
    if (channel->layers[layerIndex] == NULL) {
        layer = (SequenceLayer*)AudioScript_AudioListPopBack(&gMmSfx.layerFreeList);
...
void AudioScript_SeqLayerDisable(SequenceLayer* layer) {
    if (layer != NULL) {
        if ((layer->channel != &gMmSfx.sequenceChannelNone) && (layer->channel->seqPlayer->finished == true)) {
...
void AudioScript_SeqLayerFree(SequenceChannel* channel, s32 layerIndex) {
    ...
    if (layer != NULL) {
        AudioScript_AudioListPushBack(&gMmSfx.layerFreeList, &layer->listItem);
...
void AudioScript_SequencePlayerDisable(SequencePlayer* seqPlayer) {
    ...
    if (seqPlayer->defaultFont == gMmSfx.fontCache.temporary.entries[0].id) {
        gMmSfx.fontCache.temporary.nextSide = 1;
    } else if (seqPlayer->defaultFont == gMmSfx.fontCache.temporary.entries[1].id) {
        gMmSfx.fontCache.temporary.nextSide = 0;
    }
}
...
void AudioScript_InitLayerFreelist(void) {
    ...
    gMmSfx.layerFreeList.prev = &gMmSfx.layerFreeList;
    gMmSfx.layerFreeList.next = &gMmSfx.layerFreeList;
    gMmSfx.layerFreeList.u.count = 0;
    gMmSfx.layerFreeList.pool = NULL;

    for (i = 0; i < ARRAY_COUNT(gMmSfx.sequenceLayers); i++) {
        gMmSfx.sequenceLayers[i].listItem.u.value = &gMmSfx.sequenceLayers[i];
        gMmSfx.sequenceLayers[i].listItem.prev = NULL;
        AudioScript_AudioListPushBack(&gMmSfx.layerFreeList, &gMmSfx.sequenceLayers[i].listItem);
    }
}
...
s32 AudioScript_SeqLayerProcessScriptStep4(SequenceLayer* layer, s32 cmd) {
    ...
    switch (instOrWave) {
        ...
        default:
            ...
            semitone2 = semitone;
            {
                static int sPitchLog = 0;
                if (sPitchLog < 14) {
                    sPitchLog++;
                    char b[208];
                    snprintf(b, sizeof(b),
                             "PITCH instOrWave=%d base=%d seqT=%d chT=%d layT=%d -> semitone2=%d gPitch=%.4f",
                             instOrWave,
                             semitone2 - seqPlayer->transposition - channel->transposition - layer->transposition,
                             seqPlayer->transposition, channel->transposition, layer->transposition, semitone2,
                             (semitone2 >= 0 && semitone2 < 128) ? gPitchFrequencies[semitone2] : -1.0);
                    MmSfxSynth_Log(b);
                }
            }
            ...
            if (layer->portamento.mode != PORTAMENTO_MODE_OFF) {
                ...
                if (instrument != NULL) {
                    ...
                    tuning = 1.0f;
                    if (instOrWave >= 0xC0) {
                        layer->tunedSample = &gMmSfx.synthesisReverbs[instOrWave - 0xC0].tunedSample;
                    }
                }
                ...
                if (PORTAMENTO_IS_SPECIAL(*portamento)) {
                    speed = seqPlayer->tempo * 0x8000 / gMmSfx.maxTempo;
                    if (layer->delay != 0) {
                        speed = speed * 0x100 / (layer->delay * layer->portamentoTime);
                    }
                } else {
                    speed = 0x20000 / (layer->portamentoTime * gMmSfx.audioBufferParameters.updatesPerFrame);
                }
    ...
    if (layer->delay == 0) {
        ...
        if (layer->portamento.mode != PORTAMENTO_MODE_OFF) {
            ...
            if (PORTAMENTO_IS_SPECIAL(*portamento)) {
                speed2 = seqPlayer->tempo * 0x8000 / gMmSfx.maxTempo;
...
s32 AudioScript_SeqLayerProcessScriptStep3(SequenceLayer* layer, s32 cmd) {
    ...
    if (channel->velocityRandomVariance != 0) {
        floatDelta = layer->velocitySquare * (gMmSfx.audioRandom % channel->velocityRandomVariance) / 100.0f;
        if ((gMmSfx.audioRandom & 0x8000) != 0) {
    ...
    if (channel->gateTimeRandomVariance != 0) {
        ...
        intDelta = (layer->gateDelay * (gMmSfx.audioRandom % channel->velocityRandomVariance)) / 100;
        if ((gMmSfx.audioRandom & 0x4000) != 0) {
            intDelta = -intDelta;
        }
        ...
        if (cmd >= 0xA0) {
            ...
            switch (cmd) {
                ...
                case 0xB7: // channel: random large
                    channel->unk_22 =
                        (cmdArgs[0] == 0) ? (gMmSfx.audioRandom & 0xFFFF) : (gMmSfx.audioRandom % cmdArgs[0]);
                    break;
                ...
                case 0xBE: // channel:
                    if (cmdArgs[0] < 5) {
                        if (1) {}
                        if (gMmSfx.customSeqFunctions[cmdArgs[0]] != NULL) {
                            gAudioCustomSeqFunction = gMmSfx.customSeqFunctions[cmdArgs[0]];
...
void AudioScript_SequencePlayerProcessSequence(SequencePlayer* seqPlayer) {
    ...
    if (tempoChange > gMmSfx.maxTempo) {
        tempoChange = gMmSfx.maxTempo;
    }

    seqPlayer->tempoAcc += tempoChange;

    if (seqPlayer->tempoAcc < gMmSfx.maxTempo) {
        return;
    }

    seqPlayer->tempoAcc -= (u16)gMmSfx.maxTempo;
    seqPlayer->unk_16++;
            ...
            if (cmd >= 0xC0) {
                switch (cmd) {
                    ...
                    case 0xDD: // seqPlayer: set tempo
                        ...
                        if (seqPlayer->tempo > gMmSfx.maxTempo) {
                            seqPlayer->tempo = gMmSfx.maxTempo;
                        }
                    ...
                    case 0xCE: // seqPlayer: random value
                        cmd = AudioScript_ScriptReadU8(seqScript);
                        if (cmd == 0) {
                            seqScript->value = (gMmSfx.audioRandom >> 2) & 0xFF;
                        } else {
                            seqScript->value = (gMmSfx.audioRandom >> 2) % cmd;
                        }
                        break;
...
void AudioScript_ProcessSequences(s32 arg0) {
    ...
    gMmSfx.sampleStateOffset = (gMmSfx.audioBufferParameters.updatesPerFrame - arg0 - 1) * gMmSfx.numNotes;

    for (i = 0; i < (u32)gMmSfx.audioBufferParameters.numSequencePlayers; i++) {
        seqPlayer = &gMmSfx.seqPlayers[i];
...
void AudioScript_InitSequencePlayerChannels(s32 seqPlayerIndex) {
    SequenceChannel* channel;
    SequencePlayer* seqPlayer = &gMmSfx.seqPlayers[seqPlayerIndex];
    s32 channelIndex;
    s32 layerIndex;

    for (channelIndex = 0; channelIndex < SEQ_NUM_CHANNELS; channelIndex++) {
        seqPlayer->channels[channelIndex] =
            (SequenceChannel*)AudioHeap_AllocZeroed(&gMmSfx.miscPool, sizeof(SequenceChannel));
        if (seqPlayer->channels[channelIndex] == NULL) {
            seqPlayer->channels[channelIndex] = &gMmSfx.sequenceChannelNone;
        } else {
...
void AudioScript_InitSequencePlayer(SequencePlayer* seqPlayer) {
    ...
    for (i = 0; i < SEQ_NUM_CHANNELS; i++) {
        seqPlayer->channels[i] = &gMmSfx.sequenceChannelNone;
    }
...
void AudioScript_InitSequencePlayers(void) {
    ...
    for (i = 0; i < ARRAY_COUNT(gMmSfx.sequenceLayers); i++) {
        gMmSfx.sequenceLayers[i].channel = NULL;
        gMmSfx.sequenceLayers[i].enabled = false;
    }

    for (i = 0; i < ARRAY_COUNT(gMmSfx.seqPlayers); i++) {
        AudioScript_InitSequencePlayer(&gMmSfx.seqPlayers[i]);
    }
}
