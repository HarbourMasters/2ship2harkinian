/*
 * mm_sfx_synth_effects.cpp — VERBATIM port of MM (2ship) mm/src/audio/lib/effects.c
 * into namespace mmsfx. Only adaptations: gAudioCtx -> gMmSfx, BE16SWAP -> our
 * controllable macro, external symbols declared at top. See effects.c for docs.
 *
 * Processes seqPlayer/channel/layer volume+pan+freq each update, and implements
 * Vibrato, Portamento and the multi-point ADSR envelope — this is where MM's
 * SFX "feel" (pitch slides, tremolo, attack/decay shapes) lives.
 */
#include "mm_sfx_synth_common.h"

namespace mmsfx {

// ---- externals (defined in other ported TUs / context) ----
extern struct MmSfxAudioContext gMmSfx;
void AudioScript_SequencePlayerDisable(SequencePlayer* seqPlayer); // seqplayer.cpp

// Context accessor for audioBufferParameters (declared fully in ctx header, but
// effects.c only needs updatesPerFrameScaled). Provided via gMmSfx.
...
void AudioScript_SequenceChannelProcessSound(SequenceChannel* channel, s32 recalculateVolume, s32 applyBend) {
    ...
    if (channel->changes.s.volume || recalculateVolume) {
        // [Port][Audio] Scale volume by our sequence player volume slider
    ...
    switch (status) {
        ...
        case ADSR_STATUS_LOOP:
            ...
            switch (adsr->delay) {
                ...
                default:
                    adsr->delay *= MmSfx_GetBufParams()->updatesPerFrameScaled;
                    if (adsr->delay == 0) {
                        adsr->delay = 1;
                    }
