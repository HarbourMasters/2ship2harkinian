/*
 * mm_sfx_synth_data.cpp — verbatim audio data tables extracted from
 * Majora's Mask / 2ship (mm/src/audio/lib/data.c, plus a couple of
 * constants that live elsewhere in mm/src/audio). Used by the isolated
 * MM SFX synth (namespace mmsfx): seqplayer.c / playback.c / effects.c.
 *
 * EVERY numeric value here is copied VERBATIM from the 2S2H source — do
 * not "tidy" the floats; wrong values = wrong sound. Source line numbers
 * are noted above each table.
 *
 * NOTE ON THE ADSR DECAY TABLE:
 *   MM has NO constant source decay-rate table (there is no
 *   `gAudioDecayRates` / `sAdsrDecayTable` constant in MM). The runtime
 *   `gAudioCtx.adsrDecayTable[256]` (f32) is built PROCEDURALLY at heap
 *   init by AudioHeap_InitAdsrDecayTable(). The exact procedure is
 *   reproduced as MmSfx_InitAdsrDecayTable() at the bottom of this file
 *   so the synth init can call it. See heap.c:26-55.
 */
#include "mm_sfx_synth_common.h"

#ifndef ALIGNED
#define ALIGNED(x) /* alignment is irrelevant for a software-only synth */
#endif

namespace mmsfx {

// ============================================================================
// Wave sample arrays (data.c:312-649). Each is 256 s16 (4 harmonics x 64).
// gWaveSamples[] (data.c:652) indexes into these; index [2] is the sine wave.
// ============================================================================
...
// data.c:396 — THE SINE WAVE. gWaveSamples[2]. Vibrato curve default.
...
// data.c:523 — "Sine White Noise?" (D_801D4790, gWaveSamples[5])
...
// data.c:566 — Pulse Wave (duty cycle = 12.5%), gWaveSamples[6]
...
// data.c:609 — Pulse Wave (duty cycle = 25%), gWaveSamples[7] and [8]
...
// data.c:652 — table of pointers to the 6+ wave arrays. Header declares
// gWaveSamples[6]; MM's array actually has 9 entries (the extra pulse
// variants). We provide all 9 so harmonic indices 5..8 stay valid; the
// header extern is `s16* gWaveSamples[6]` but the definition can be larger
// (only the first 6 are reachable through the declared bound; effects.cpp
// uses index 2). Keeping all 9 matches MM exactly.
s16* gWaveSamples[] = {
...
// ============================================================================
// Portamento / pitch-bend frequency tables (data.c:657, 686). 256 f32 each.
// ============================================================================

// data.c:657 — gBendPitchOneOctaveFrequencies (256 f32). REQUIRED by effects.c
// portamento (one octave = +/- 12 semitones around index 128 = 1.0f).
...
// data.c:686 — gBendPitchTwoSemitonesFrequencies (256 f32). +/- 2 semitones
// around index 128 = 1.0f.
...
// ============================================================================
// gPitchFrequencies (data.c:715). 128 f32. This is MM's per-semitone
// "note frequency" table — the equivalent of OOT's gNoteFrequencies.
// seqplayer.c converts a note's semitone index -> freqScale via this table
// (seqplayer.c:920-976: gPitchFrequencies[semitone] * tuning).
...
// ============================================================================
// Short-note tables (data.c:846, 850). 16 u8 each. Used by seqplayer.c.
// ============================================================================
...
// ============================================================================
// Pan / volume tables (data.c:965, 981, 997). 128 f32 each. Used by
// playback.c to compute per-note left/right volumes.
//   - gHeadsetPanVolume  : headset (SOUNDMODE_HEADSET) pan curve
//   - gStereoPanVolume   : stereo (SOUNDMODE_STEREO) pan curve
//   - gDefaultPanVolume  : default / mono pan curve
// MM has NO "gPan" / "gHeadsetPanQuantization" tables; those are OOT names.
// ============================================================================
...
// ============================================================================
// Tatum / tempo.
//   TATUMS_PER_BEAT == 48 (z64audio.h:20). MM's tempo is stored internally as
//   (BPM * TATUMS_PER_BEAT). gTatumsPerBeat in MM is gAudioTatumInit[1], whose
//   value IS TATUMS_PER_BEAT (session_config.c:4-7). There is NO
//   gTempoInternalToExternal table in MM (that is an OOT-only name); the
//   external BPM is recovered by simple division: BPM = tempo / TATUMS_PER_BEAT
//   (see sequence.c:628). We expose the constant + the tatum-init array.
// ============================================================================
...
// ============================================================================
// ADSR decay-rate table — there is NO source constant in MM.
//
// gAudioDecayRates: the common header declares `extern u8 gAudioDecayRates[][16]`
// but MM has NO such constant. The runtime f32 table gAudioCtx.adsrDecayTable[256]
// is built procedurally (see MmSfx_InitAdsrDecayTable below). We provide a single
// dummy row so the extern resolves at link time; it is NOT used for ADSR. If the
// header extern is dropped, delete this too.
// ============================================================================
u8 gAudioDecayRates[][16] = {
    { 0 },
};

// ----------------------------------------------------------------------------
// Procedural ADSR decay table builder. VERBATIM port of MM's
// AudioHeap_CalculateAdsrDecay + AudioHeap_InitAdsrDecayTable (heap.c:26-55).
//
// Call once after `updatesPerFrameInvScaled` is known (it is part of
// AudioBufferParameters). `outTable` must hold 256 f32. The runtime table is
// what playback.c indexes as gAudioCtx.adsrDecayTable[decayIndex]
// (playback.c:577-579) to set adsr.fadeOutVel.
// ----------------------------------------------------------------------------
f32 MmSfx_CalculateAdsrDecay(f32 updatesPerFrameInvScaled, f32 scaleInv) {
    // heap.c:26-28
    return 256.0f * updatesPerFrameInvScaled / scaleInv;
}
...
void MmSfx_InitAdsrDecayTable(f32* outTable, f32 updatesPerFrameInvScaled) {
    ...
    // heap.c:36-40
    outTable[255] = MmSfx_CalculateAdsrDecay(updatesPerFrameInvScaled, 0.25f);
    outTable[254] = MmSfx_CalculateAdsrDecay(updatesPerFrameInvScaled, 0.33f);
    outTable[253] = MmSfx_CalculateAdsrDecay(updatesPerFrameInvScaled, 0.5f);
    outTable[252] = MmSfx_CalculateAdsrDecay(updatesPerFrameInvScaled, 0.66f);
    outTable[251] = MmSfx_CalculateAdsrDecay(updatesPerFrameInvScaled, 0.75f);

    // heap.c:42-44
    for (i = 128; i < 251; i++) {
        outTable[i] = MmSfx_CalculateAdsrDecay(updatesPerFrameInvScaled, (f32)(251 - i));
    }

    // heap.c:46-48
    for (i = 16; i < 128; i++) {
        outTable[i] = MmSfx_CalculateAdsrDecay(updatesPerFrameInvScaled, (f32)(4 * (143 - i)));
    }

    // heap.c:50-52
    for (i = 1; i < 16; i++) {
        outTable[i] = MmSfx_CalculateAdsrDecay(updatesPerFrameInvScaled, (f32)(60 * (23 - i)));
    }
