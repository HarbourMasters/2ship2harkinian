...
void EnFsn_HandleConversationBackroom(EnFsn* this, GlobalContext* globalCtx) {
    switch (this->textId) {
        case 0:
            if (!(gSaveContext.save.weekEventReg[0x50] & 0x10)) {
                this->textId = 0x29E0;
                break;
            } else {
                this->textId = 0x29E4;
        ...
        case 0x29E0:
            if (INV_CONTENT(ITEM_MASK_KEATON) == ITEM_MASK_KEATON) {
                ...
                gSaveContext.save.weekEventReg[0x50] |= 0x10;
                this->textId = 0x29F1;
                break;
            } else {
                this->textId = 0x29E1;
...
s16 EnFsn_GetThirdDayItemId(void) {
    if (!(gSaveContext.save.weekEventReg[0x21] & 4) && CURRENT_DAY == 3) {
        if (!(gSaveContext.save.weekEventReg[0x21] & 8) && !(gSaveContext.save.weekEventReg[0x4F] & 0x40)) {
...
s32 EnFsn_HasItemsToSell(void) {
    if (CURRENT_DAY != 3) {
        ...
        if (((gSaveContext.save.roomInf[126][5] & 0xFF000000) >> 0x18) ||
            ((gSaveContext.save.roomInf[126][5] & 0xFF0000) >> 0x10) || !(gSaveContext.save.weekEventReg[0x21] & 4)) {
            return true;
        }
        return false;
    }
...
void EnFsn_GetShopItemIds(EnFsn* this) {
    u32 stolenItem1 = (gSaveContext.save.roomInf[126][5] & 0xFF000000) >> 0x18;
    u32 stolenItem2 = (gSaveContext.save.roomInf[126][5] & 0xFF0000) >> 0x10;
    s16 itemId;
...
s32 EnFsn_FacingShopkeeperDialogResult(EnFsn* this, GlobalContext* globalCtx) {
    switch (globalCtx->msgCtx.choiceIndex) {
        case 0:
            ...
            } else if (gSaveContext.save.weekEventReg[0x21] & 4) {
    ...
    switch (item->canBuyFunc(globalCtx, item)) {
        ...
        case CANBUY_RESULT_SUCCESS_1:
            ...
            if (this->stolenItem1 == this->cursorIdx) {
                gSaveContext.save.roomInf[126][5] &= ~0xFF000000;
            } else if (this->stolenItem2 == this->cursorIdx) {
                gSaveContext.save.roomInf[126][5] &= ~0xFF0000;
            }
