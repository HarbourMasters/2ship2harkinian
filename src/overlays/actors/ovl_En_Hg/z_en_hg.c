...
void EnHg_Init(Actor* thisx, GlobalContext* globalCtx) {
    ...
    if ((gSaveContext.save.weekEventReg[0x4B] & 0x20) || (gSaveContext.save.weekEventReg[0x34] & 0x20)) {
        ...
        if (this->cutscenes[3] != globalCtx->csCtx.npcActions[actionIndex]->unk0) {
            ...
            switch (globalCtx->csCtx.npcActions[actionIndex]->unk0) {
                ...
                case 5:
                    ...
                    break;
                case 6:
                    gSaveContext.save.weekEventReg[0x4B] |= 0x20;
                    Actor_MarkForDeath(&this->actor);
                    break;
            }
        } else {
        ...
        if (globalCtx->msgCtx.ocarinaMode == 3) {
            ...
            if (this->actor.xzDistToPlayer < 60.0f && fabsf(this->actor.playerHeightRel) < 40.0f) {
                if ((this->actionFunc != func_80BCF8A0) && (this->actionFunc != func_80BCF95C)) {
                    if (!(gSaveContext.save.weekEventReg[0x3D] & 2)) {
                        gSaveContext.save.weekEventReg[0x3D] |= 2;
                        this->unk218 = 0;
                    } else {
                        this->unk218 = 2;
                    }
